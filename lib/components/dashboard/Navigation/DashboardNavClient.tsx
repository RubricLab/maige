		...
		<div className="sticky top-0 z-50 flex w-full select-none flex-row items-center justify-between pt-4 pb-5 backdrop-blur-sm">
			<div className="flex items-center gap-4">
				...
				<div className="inline-flex items-center justify-center gap-1.5">
					...
					{params.projectId && (
						<span className="mb-0.5 select-none px-1 pr-2 font-light text-xl opacity-30">/</span>
					)}
					{params.projectId && (
						<ProjectSelect projects={projects} projectId={params.projectId} teamSlug={params.slug} />
					)}
				</div>
			</div>
			<div className="flex items-center gap-4">
				...
				<DropdownMenu>
					...
					<DropdownMenuContent className="mt-2 mr-6 w-[235px]">
						{(user.name || user.email) && (
							<>
								<DropdownMenuItem disabled={true}>{user.name ? user.name : user.email}</DropdownMenuItem>
								<DropdownMenuSeparator />
							</>
						)}
