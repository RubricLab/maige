...
import type { Label, Repository } from '~/types'
...
| **${title}** | 🟡 Pending ([inspect](${env.NEXTAUTH_URL}/${teamSlug}/usage/runs?proj=${projectId})) | | ${getFormattedDate()} |`
	...
	async function updateTracking(_status: string) {
		await editComment({
			octokit,
			commentId,
...
| **${title}** | ❌ Error ([inspect](${env.NEXTAUTH_URL}/${teamSlug}/usage/runs?proj=${projectId})) | Errored | ${getFormattedDate()} |`
	...
	async function completeTracking(_status: string) {
		await editComment({
			octokit,
			commentId,
			comment: `**Engineer dispatched.** See details on the [Maige dashboard](${env.NEXTAUTH_URL}).
...
export async function labelIssue({
	...
	const labelIds = allLabels.filter(label => labelNames.includes(label.name)).map(label => label.id)
 ...
 */
export const getMainBranch = async (repoFullName: string, accessToken?: string) => {
...
export async function handleAddOrDeleteProjects({
	...
	try {
		...
		const { repositories_added: addedRepos, repositories_removed: removedRepos } = payload
		...
		const newRepos = addedRepos.filter((repo: Repository) => {
			return !user.projects.some((project: { name: string | null }) => project.name === repo.name)
		})
